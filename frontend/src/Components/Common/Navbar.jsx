...
import { useSelector } from "react-redux"
   ...
   const Navbar = () =>{
    ...
    <nav className="container mx-auto flex items-center justify-between py-4 px-6">
     {/* left logo */}
     <div><Link to ="/" className="text-2xl font-medium">Premiere</Link></div>
     ...
     <div className="flex items-center space-x-4">
        {user && user.role === "admin" && ( <Link to ="/admin" className="bg-black px-2 rounded text-sm text-white">Admin</Link>)}
        ...
        <button onClick={toggleCartDrawer} className="relative hover:text-black">
            < HiOutlineShoppingBag className="h6 w-6 text-gray-700"/>
            {cartItemCount > 0 && (
    ...
    {/* mobile navigation */}
    <div className={`fixed top-0 left-0 w-3/4 sm:-1/2 md:w-1/3 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${navDrawerOpen?"translate-x-0":"-translate-x-full" }`}>
    ...
    <div className="p-4">
        <h2 className="text-xl font-semibold md-4">Menu</h2>
        <nav> 
